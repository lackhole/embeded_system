...
#ifndef WATCHER_NETWORK_PACKET_H_
#define WATCHER_NETWORK_PACKET_H_
...
class StatusCode {
 public:
  ...
  StatusCode() = default;
  StatusCode(code v) : val_(v) {}

  bool operator==(const std::string& value) const { return value == std::to_string(val_); }
  bool operator==(code value) const noexcept { return value == val_; }
 ...
 private:
  ...
  [[nodiscard]] string_view_type raw_header() const noexcept {
    return {buffer_.data() + to_byte(kPacketHeaderSizeBit),
            raw_header_size() < to_byte(kPacketHeaderSizeBit) ? 0 : raw_header_size() - to_byte(kPacketHeaderSizeBit)};
  }
...
#endif // WATCHER_NETWORK_PACKET_H_
